...
*/ const $name = '东东农场';
let cookie = '', jdFruitShareArr = [], newShareCodes;
...
let isLogin;
let $UserName, $index = 0, $nickName = '', $retry;
let farmInfo;
let farmTask;
let signResult;
let goalResult;
...
async function farmAssistInit() {
    const functionId = arguments.callee.name.toString();
    farmAssistResult = await request(functionId, {
        "version": 14,
        "channel": 1,
        "babelChannel": "120"
    });
...
async function receiveStageEnergy() {
    const functionId = arguments.callee.name.toString();
    $receiveStageEnergy = await request(functionId, {
        "version": 14,
        "channel": 1,
        "babelChannel": "120"
    });
 ...
 */ async function browseAdTaskForFarm(advertId, type) {
    const functionId = arguments.callee.name.toString();
    if (type === 0) {
        browseResult = await request(functionId, {
            advertId,
            type,
            "version": 14,
            "channel": 1,
        ...
        browseRwardResult = await request(functionId, {
            advertId,
            type,
            "version": 14,
            "channel": 1,
...
async function request(function_id, body = {}, timeout = 1000) {
    wait(timeout);
...
async function task(function_id, body = {}, isPOST) {
